...
function desencriptar() {
  consulta = textoUsuario.value.toLowerCase();

  //comprobacion de que no este vacio el input
  if (textoUsuario.value.trim() != "") {
    for (const propiedad in llaves) {
      consulta = consulta.replaceAll(llaves[propiedad], propiedad);
    }
    mostrarVentana();
  } else {
...
function mostrarVentana() {
  ...
  mostrarTexto.innerHTML = consulta;
  textoUsuario.value = "";
}
